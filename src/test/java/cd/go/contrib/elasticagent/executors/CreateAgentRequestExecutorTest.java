...
public class CreateAgentRequestExecutorTest {
    ...
    public void shouldAskDockerContainersToCreateAnAgent() throws Exception {
        ...
        verify(agentInstances).createIfNecessary(eq(request), eq(request.clusterProfileProperties()), eq(pluginRequest), any(ConsoleLogAppender.class));
    }
    ...
    public void shouldLogErrorMessageToConsoleIfAgentCreateFails() throws Exception {
        ...
        when(agentInstances.createIfNecessary(any(), any(), any(), any())).thenThrow(new RuntimeException("Ouch!"));
